...
for (let i = 0; i < item_names.length; i+=1) {
    [i].innerHTML = item_names[i].innerHTML = item_desc[i]


let item_descr_objct = dokument.querySelectorAll('.item_deskr_objekts')
let item_desc = ['Тарілка з принтом Марса. Доступна в червоному та білому кольорах']
for (let i = 0; i <item_name_objects.length; i += 1) {
    item_descr_objekt[i]].innerHTML = item_desc[i]

let item_descr_objct = dokument.querySelectorAll('.item_deskr_objekts')
let item_prise = ['1000','1000','2500','5000']
for (let i = 0; i < item_names.length; i+=1) {
    item_prise_object[i].innerHTML = item_prise[i]
